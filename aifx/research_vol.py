...
def report(res: dict) -> str:
    ...
    L += ["", "ニューヨーク時間の曜日×時間 (各枠は時間ごとの値に20本分の重みで引き寄せる) は、1時間先・4時間先で調整期間と検証期間の"
          ...
          "本番に採用しました。", "",
          "採用後、予測の計算 (forecaster.py) が値動きの大きさの計算にモデル用の直近3,000本しか渡しておらず、曜日×時間の割合が"
          "実際には約3,000本で測られていたことが分かりました (research/har.md の調査で発見)。その「修正前の本番」の行のとおり、"
          "3,000本では1時間先の検証期間が UTC の版より悪く、6,000本で測ると1時間先・4時間先とも両方の期間で良くなります (24時間先はほぼ同じ)。"
          "そのため、値動きの大きさの計算には6,500本を渡すように直しました (engine.vol_bars)。", ""]
    return "\n".join(L)
