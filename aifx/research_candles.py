...
def evaluate(tf: str, log=print) -> dict:
    ...
    for code in PAIRS:
        ...
        for t in range(start, n - steps, every):
            cs, _ = scenario.candles(tf, df.iloc[: t + 1], steps, c[t], minutes)
            if not cs:
                continue
    ...
    R = pd.DataFrame(rows, columns=["j", "pd", "ad", "pb", "ab", "pr", "ar", "atr"])
...
def report(res: dict, ml: dict | None = None) -> str:
    L = ["# 予想ローソク足の精度", "",
         ...
         "| 時間足 | 何本先 | 件数 | 終値の方向の的中率 | 陽線・陰線の的中率 | 足の大きさの相関 (ATR) | 足の大きさの誤差 (ATR比) |",
         "|---|---|---|---|---|---|---|"]
    for tf, r in res.items():
        name = {"1h": "1時間足", "1d": "日足", "15m": "15分足"}[tf]
        for j, x in r["h"].items():
            L.append(f"| {name} | {j} | {x['n']:,} | {x['dir_hit']:.1%} | {x['color_hit']:.1%} | {x['size_corr']:.2f} ({x['atr_corr']:.2f}) | "
                     f"{x['size_mae_vs_atr'] - 1:+.0%} |")
    L += ["", "- 足の大きさは、単純な平均値幅 (直近14本の ATR) より正確に予想できています (誤差がマイナスなら ATR より小さい)。",
          ...
          "- 方向 (終値が上か下か、陽線か陰線か) は、どの時間足でも偶然 (50%) と区別できません。予想ローソク足の実体の向きや並びは、"
          "過去の似た局面の一例であって、当てにできるものではありません。", ""]
    if ml:
        L += ["## 機械学習による方向の予測", "",
